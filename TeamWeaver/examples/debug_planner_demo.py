...
def run_planner(config, dataset: CollaborationDatasetV0 = None, conn=None):
    ...
    def log_time(message):
        with open(config.paths.timing_log_file, 'a') as log_file:
    ...
    config.paths.timing_log_file = os.path.join(config.paths.results_dir, 'timing_log.txt')
    with open(config.paths.timing_log_file, 'w') as log_file:
        log_file.write("Timing Log\n")
    ...
    if config.env == "habitat":
        ...
        print("##################### Config.evaluation.save_video: ##################")
        print(config.evaluation.save_video)
        print("######################################################################")
        ...
        try:
            ...
            log_time("Initialized perception and world graph")
        except Exception:
    ...
    log_time(f"Initialization completed in {init_duration:.2f} seconds")
    ...
    log_time(f"Planner construction completed in {planner_duration:.2f} seconds")
        ...
        for run_id in range(config.num_runs_per_episode):
            for _ in range(num_episodes):
                ...
                try:
                    ...
                    log_time(f"Episode {episode_id}, Run {run_id} executed in {execution_duration:.2f} seconds")
