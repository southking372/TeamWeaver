...
def quaternion_to_yaw(q):
    """
    Converts a quaternion [x, y, z, w] or [w, x, y, z] to yaw angle (theta) in radians.
    Adjust order based on Habitat's convention. Assuming [x, y, z, w] here.
    """
    try:
        q_np = np.array(q)
        ...
        if np.isclose(norm, 0):
             print("Warning: Zero quaternion encountered. Returning 0 yaw.")
             return 0.0
        q_normalized = q_np / norm

        rotation = R.from_quat(q_normalized)
        euler_angles = rotation.as_euler('xyz', degrees=False) # Use 'xyz' or 'zyx' etc. as appropriate
        return euler_angles[1] # Or euler_angles[0] or euler_angles[2] depending on convention
    except Exception as e:
        print(f"Error converting quaternion {q} to yaw: {e}")
