...
class LLMPlanner(Planner):
    ...
    def replan(
        ...
        try:
            ...
            # global_guidance = self.prompt_builder.build_global_guidance_overview(
            #     current_phase_tasks,
            #     self.perception_connector.phase_manager.task_execution_phases,
        ...
        try:
            ...
            prompt_wait = "Take care of your spatial position and navigate to target before pick or place logic."
            prompt_miner = self.prompt_builder.prepare_llm_prompt(self.curr_prompt, prompt_wait)
            
            # agents_summary = self.task_helper.get_agents_summary("planning")
            # prompt_emos = self.prompt_builder.prepare_llm_prompt(self.curr_prompt, agents_summary)
            # Debug info
            # if agents_summary:
            #     print(f"[DEBUG] Agents Summary injected ({len(agents_summary)} chars):")
            ...
            if (self.replanning_count <= 5):
                ...
                last_raw_response = raw_response
                raw_response = compare_response
            # elif (self.perception_connector.phase_manager.current_phase_index + 1 <= (len(self.perception_connector.phase_manager.task_execution_phases) / 2)):
            #     print(f"[DEBUG-LYP-v3] current_phase_index: {self.perception_connector.phase_manager.current_phase_index + 1}")
            #     print(f"[DEBUG-LYP-v3] all_phase_index: {len(self.perception_connector.phase_manager.task_execution_phases)}")
            #     compare_response = self.llm.generate(
            #         prompt_miner,
            #         self.stopword,
            #         generation_args={
