...
class AgentResume:
    """
storageAgentdetailed information“CV”, for intelligent task allocation.
Contains static capabilities, performance traits and dynamic states.
    ...
    position: Optional[np.ndarray] = None #fromworld_staterenew
...
class TaskHelper:
    ...
    def _generate_prompt_summary(self) -> str:
        """Generate suitableLLMDetailed team summary of planning"""
    ...
    def update_resumes_from_context(
        ...
        """
Update everything based on the latest world state, feedback, and actions performed in the previous stepAgentresume.
This method should be used in everyreplanCalled at the beginning of a cycle to enable feedback-based learning.
        """
        for agent_id, resume in self.agent_resumes.items():
            #fromworld_stateUpdate location information
...
Based on the execution feedback of specific actions, performance characteristics are updated in a refined manner to implement a simple learning mechanism.
    ...
    def _initialize_agent_resumes(self) -> Dict[int, AgentResume]:
        """
initialize allAgentof“CV”. In practical applications, this information can be loaded from configuration files.
        """
        resumes = {}
    ...
    def _calculate_assignment_score(self, task: Dict[str, Any], resume: AgentResume) -> float:
        """
Core scoring function: Calculate aAgentfor a specific task“adaptability score”。
The higher the score, the better the fit. Returning 0 means it cannot be executed.
        ...
        #2. Matching performance characteristics(soft constraints/Bonus points)
        ...
        #3. Dynamic state impact(penalty item), achieve load balancing
        score -= resume.current_task_load * 0.25

        #Punishment based on spatial distance
        #objective facts as planning factors, try asAgentPreference
        ...
        return max(0.1, score) #Make sure you can do itAgentAt least 0.1 points
    ...
    def _distribute_tasks_with_scoring(
        ...
        """
Use a rating-based system to intelligently assign a set of task instances to a group ofAgent。
        """
        assignments: Dict[int, List[Dict[str, Any]]] = {i: [] for i in agent_ids}
        
        #real time updatesAgentdynamic information(task load)
        for agent_id in agent_ids:
            self.agent_resumes[agent_id].current_task_load = 0
            #Here you can alsoworld_staterenewagent_resumes[agent_id].position

        #Go through each task and find the most suitable one for itAgent
        for task in tasks:
            scores = {
            ...
            #Find the one with the highest scoreAgent
            if any(s > 0 for s in scores.values()):
                best_agent_id = max(scores, key=scores.get)
            else:
                # Fallback:if allAgentIf neither can be executed, it will be assigned to the first
            ...
            #update thisAgenttask load to provide the correct status for the next task assignment
    ...
    def get_aptitude_matrix(self, tasks: List[Dict[str, Any]], agents: List["Agent"]) -> np.ndarray:
        """
Generate an adaptability matrix(Aptitude Matrix), indicating eachAgentFitness score for each task.
The matrix dimensions are(n_agents, n_tasks)。
The higher the score, the moreAgentThe more suitable it is for performing the task.
        """
        n_agents = len(agents)
        n_tasks = len(tasks)
    ...
    def assign_tasks_for_phase(
        ...
        """
according toMIQPmacro-allocation results, combined with“CV”The scoring system is used for final assignment of task instances.
        """
        ...
        if alpha is None or not np.any(alpha > 0.5):
            ...
            #whenMIQPWhen there is no solution, all tasks in the current stage will be assigned through the scoring system
            agent_ids = [agent.uid for agent in agents]
            return self._distribute_tasks_with_scoring(current_phase_tasks, agent_ids)

        #Normal process:MIQPThere is a solution
