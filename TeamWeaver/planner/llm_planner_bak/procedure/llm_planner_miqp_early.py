...
class LLMPlanner(Planner):
    ...
    def replan(
        ...
        try:
            ...
            # This is a robust way to inject new context for the LLM's turn.
            if miqp_guidance:
                prompt_for_llm += (
            ...
            if self.planner_config.get("constrained_generation", False):
                ...
                raw_response = self.llm.generate(prompt_for_llm, self.stopword)

            print(f"###################[DEBUG-LYP] raw_response: \n{raw_response}")
            print(f"###################[DEBUG-LYP] compare_response: \n{compare_response}")
            print(f"###################[DEBUG-LYP] miqp_response: \n{miqp_response}")
            ...
            response = self.format_response(response, self.end_expression)
            
            print(f"[DEBUG-LYP] Formatted LLM response: \n{response}")

            if not response or response == "Thought:":
        ...
        # **CRITICAL**: 添加调试输出，显示使用的prompt长度信息
        ...
        try:
            high_level_actions = self._parse_high_level_actions(response)
    ...
    def get_next_action(
        ...
        **ENHANCED** 与MIQP replan集成的主要入口方法
        ...
        if self.replan_required:
            ...
            #         high_level_actions = self._adjust_actions_with_phase_awareness(
            #             high_level_actions, 
