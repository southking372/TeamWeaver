...
class LLMPlanner(Planner):
    ...
    def replan(
        ...
        if self.miqp_solver_wrapper.scenario_params == None:
            print("[DEBUG] Initializing miqp_wrapper scenario parameters...")
        ...
        # print(f"\n#######################[DEBUG-LYP] World Graph Description:\n{world_description}\n")
        ...
        # === Action Generation and Validation Loop ===
        llm_response = ""
        validation_feedback = None
        max_retries = 2
        adjusted_actions = {}
        for attempt in range(max_retries):
            print(f"\n[Cycle {attempt + 1}/{max_retries}] Action Generation and Validation")
            # === Step 10: Build Phase-Aware Prompt ===
            print(f"[Step 10.{attempt+1}] Building phase-aware enhanced prompt...")
            try:
            ...
            print(f"[Step 11.{attempt+1}] Generating actions via LLM with complete feedback history...")
            try:
                prompt_for_llm = self.prompt_builder.prepare_llm_prompt(self.curr_prompt, miqp_guidance)
                # print(f"[DEBUG-LYP] miqp_guidance (attempt {attempt+1}): \n{miqp_guidance}")
                ...
                print(f"###################[DEBUG-LYP] raw_response (attempt {attempt+1}): \n{raw_response}")
                llm_response = self.format_response(raw_response, self.end_expression)
                if not llm_response or llm_response.strip() == "Thought:":
                    print(f"[WARNING] Empty LLM response received after formatting. Using fallback.")
            ...
            print(f"[Step 12.{attempt+1}] Parsing and validating actions with closed-loop feedback...")
            try:
                original_actions = self.action_manager.parse_high_level_actions(llm_response, self._agents)
                ...
                for agent_id in adjusted_actions:
                    original_action = original_actions.get(agent_id)
                    adjusted_action = adjusted_actions.get(agent_id)
                    if str(original_action) != str(adjusted_action):
                        corrections_made = True
                        original_action_str = f"{original_action[0]}[{original_action[1]}]" if original_action and len(original_action) > 1 else "None"
                        adjusted_action_str = f"{adjusted_action[0]}[{adjusted_action[1]}]" if adjusted_action and len(adjusted_action) > 1 else "None"
                        feedback_lines.append(f"For Agent {agent_id}, action '{original_action_str}' was invalid and corrected to '{adjusted_action_str}'.")
                if not corrections_made:
                    print("[INFO] Validation successful, no corrections needed. Exiting loop.")
                    break 
                else:
                    validation_feedback = "The previous plan had issues and was auto-corrected:\n" + "\n".join(feedback_lines)
                    print(f"[WARNING] Validation found issues. Retrying with feedback:\n{validation_feedback}")
                    if attempt == max_retries - 1:
                        print("[ERROR] Max retries reached. Using the last corrected plan.")
            except Exception as e:
                print(f"[ERROR] Action validation failed during loop: {e}")
                try:
                    adjusted_actions = self.action_manager.parse_high_level_actions(llm_response, self._agents)
                except:
                    adjusted_actions = {}
                break
        # === After Loop: Finalize Actions and Response ===
        
        self.execution_manager.record_execution_context(
            adjusted_actions,
            observations,
            self.get_last_agent_positions_miqp(world_graph)
        )
        
        validation_summary = self.action_manager.get_action_validation_summary()
        print(f"[DEBUG-LYP] FINAL CLOSED-LOOP FEEDBACK Summary:")
        ...
        print(f"[DEBUG] FINAL Validated actions after loop:")
        for agent_id, action in adjusted_actions.items():
            if action and len(action) >= 3:
                print(f"  Agent {agent_id}: {action[0]}({action[1]}) → {action[2]}")

        print("[DEBUG-LYP] Reconstructing llm_response from final adjusted actions to ensure consistency.")
    ...
    def get_next_action(
        ...
        self.replan_required = self.replan_required or any(responses.values())
