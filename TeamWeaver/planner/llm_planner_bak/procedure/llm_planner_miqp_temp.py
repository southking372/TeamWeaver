...
class LLMPlanner(Planner):
    ...
    def get_last_agent_positions_miqp(self) -> Dict[int, Dict[str, Any]]:
        agent_poses = {}
        agents = self.env_interface.full_world_graph.get_agents()
        for agent in agents:
            ...
            try:
                ...
                # rotation_quat = agent.get_property("rotation") # 目前rotation_quat对于agent无法获取，应采用其他方式获取
    ...
    def _setup_replan_state(self) -> Tuple[np.ndarray, Dict[str, Any]]:
        """
        ...
        agent_poses = self.get_last_agent_positions_miqp()
    ...
    def replan(
        ...
        x, world_state = self._setup_replan_state()
