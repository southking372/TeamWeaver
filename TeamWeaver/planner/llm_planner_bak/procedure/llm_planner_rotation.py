...
class LLMPlanner(Planner):
    ...
    def get_last_agent_positions_miqp(self, world_graph: Dict[int, "WorldGraph"]) -> Dict[int, Dict[str, Any]]:
        ...
        print(f"[DEBUG] agents_from_graph: {agents_from_graph}")
        ...
        for agent_entity in agents_from_graph:
            ...
            try:
                # Use the agent_entity from the graph to get pose properties
                position = agent_entity.get_property("translation")
                rotation_quat = agent_entity.get_property("rotation")
            ...
            if agent_name in agent_map:
                ...
                agent_poses[agent_uid] = {'position': position, 'rotation': rotation_quat}
            else:
                try:
        ...
        print(f"[DEBUG-LYP-v3] Extracted agent poses: {agent_poses}")
        return agent_poses
    ...
    def _setup_replan_state(self, world_graph: Dict[int, "WorldGraph"]) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        def quaternion_to_yaw(quat: np.ndarray) -> float:
            """Calculates yaw from a quaternion."""
            # x, y, z, w
        ...
        for i, agent in enumerate(self._agents):
            ...
            if agent_id in agent_poses:
                ...
                rotation_quat = np.array(pose_info['rotation'])
                
                x[0, i] = position[0]  # x
                x[1, i] = position[2]  # z is used as y for 2D planning
                x[2, i] = quaternion_to_yaw(rotation_quat)
            else:
    ...
    def replan(
        ...
        try:
            ...
            self.task_helper.update_resumes_from_context(world_state, self.latest_agent_response)
        ...
        print(f"[Step 4/12] Getting current phase tasks...")
        current_phase = self.perception_connector.get_current_phase_tasks()
        ...
        try:
            if self.miqp_solver_wrapper.scenario_params is not None:
                ...
                phase_task_info = {
                    ...
                    'indices': active_task_indices,  # Corresponds to instance indices
                    'types': active_task_types,
                    'n_phase_tasks': n_instances,
                    'n_total_tasks': n_instances  # MIQP now solves for instances
                }
        ...
        try:
            if self.miqp_solver_wrapper.scenario_params is not None and len(current_phase_tasks) > 0:
                # Dynamically generate the robot bounds for the current set of task instances
        ...
        try:
            ...
            # 5. Form the complete turn, then remove text after the end expression.
        ...
        # === Step 12: Parse High-Level Actions ===
        print(f"[Step 12/12] Parsing high-level actions...")
        try:
